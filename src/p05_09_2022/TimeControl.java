...
//	U glavnom programu kreirati video plejer i izvrsiti neke akcije nad njim.
	...
	public void izvrsiAkciju(VideoPlayer player) {
		if (unapred) {
			double sledeceVreme = player.getTrenutnoVreme() + 15;
			if (sledeceVreme > player.getDuzinaVidea()) {
				sledeceVreme = player.getDuzinaVidea();
			}
			player.setTrenutnoVreme(sledeceVreme);
		} else {
			double sledeceVreme = player.getTrenutnoVreme() - 15;
			if (sledeceVreme < player.getDuzinaVidea()) {
				sledeceVreme = 0;
